...
| 连排排期 | 选排练日、开始时间与连排场次，操耍人从已派角色汇总、用时按场次时长合计；安排前逐人核对已排时段（含同一天已排连排），撞期不记录并点明谁撞在哪一段；排期按排练日 + 开始时间列出，可取消释放时段 |
...
**冲突拦截**：指派操耍人时，会依据该人已排时段与同场其他影人操耍人的时段做重叠判定，冲突的候选人在下拉中直接禁用并给出拦截原因；操耍人自身时段互相重叠也会高亮预警。安排连排时同样先做撞期核对：参与操耍人的已排时段、以及同一天已排的连排（全社）都算占用，有人撞期这场连排就不记下来，页面逐条点明谁撞在哪一段。
...
└── frontend/                   # 前端源码
    ...
    └── src/
        ├── types/              # play.ts scene.ts role.ts operator.ts cue.ts runThrough.ts
        ├── stores/             # playStore.ts sceneStore.ts operatorStore.ts runThroughStore.ts（Zustand）
        ...
        ├── pages/              # PlayList.tsx SceneBoard.tsx RunThroughBoard.tsx RoleAssign.tsx CueTimeline.tsx OperatorList.tsx
...
| `/plays/:id/runthroughs` | 连排排期 | RunThrough、Scene、ShadowRole、Operator |
...
| RunThrough 连排 | `src/types/runThrough.ts` | id、playId、weekday、startMinute、durationMinute、sceneIds、sceneTitles、operatorIds、note |

---

## 六、数据存储说明

- **IndexedDB（Dexie）**：`src/utils/db.ts` 封装全部读写，数据库名 `gbshadowplay`，当前结构版本 **3**（v2 补齐 `revision` 行修订号、兜底 `createdAt/updatedAt`；v3 新增 `runThroughs` 连排排期表）。
- **localStorage**：`src/utils/localStore.ts` 统一封装界面偏好（最近打开的剧目、场次页「只看本次勾选」开关等）。
- **首次打开**：数据库为空时自动灌入示例班社数据（3 出剧目 / 6 个场次 / 12 个影人角色 / 4 位操耍人 / 10 处锣鼓点 / 1 场连排），保证界面开箱即有内容可点。
