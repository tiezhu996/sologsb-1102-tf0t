...
| 连排排期 | 选排练日、开始时间与连排场次，参与操耍人从已派角色汇总、用时按场次时长合计；安排前自动对参与人的已排时段（含同一天已排的连排），撞期不记下并点明谁撞在哪一段；排期按排练日与开始时间列出，可取消，取消后时段重新空出 |
...
**冲突拦截**：指派操耍人时，会依据该人已排时段与同场其他影人操耍人的时段做重叠判定，冲突的候选人在下拉中直接禁用并给出拦截原因；操耍人自身时段互相重叠也会高亮预警。安排连排时，会把参与操耍人的已排时段与同一天已排的连排一并核对，有人撞期则整场连排不记下来，页面逐条点明谁撞在哪一段。
...
└── frontend/                   # 前端源码
    ...
    └── src/
        ...
        ├── pages/              # PlayList.tsx SceneBoard.tsx RehearsalPlan.tsx RoleAssign.tsx CueTimeline.tsx OperatorList.tsx
...
| `/plays/:id/rehearsals` | 连排排期 | Rehearsal、Scene、Operator |
...
| Rehearsal 连排 | `src/types/rehearsal.ts` | id、playId、weekday、startMinute、sceneIds、note（用时与参与人按场次/角色实时派生） |

---

## 六、数据存储说明

- **IndexedDB（Dexie）**：`src/utils/db.ts` 封装全部读写，数据库名 `gbshadowplay`，当前结构版本 **3**（v2 补齐 `revision` 行修订号、兜底 `createdAt/updatedAt`；v3 新增 `rehearsals` 连排表）。
